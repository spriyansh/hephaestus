...
dataFerry <- function(){
  ...
  # Tab Panel
  tabPanel("Fetch", value = "Fetcher",tags$h2("Fetch Datasets from Gene Expression Omnibus (GEO)"),hr(),# tabPanel-2-cont
           
           # Sidebar Panel
         sidebarPanel(fluidRow(tags$h3("Control-Panel"), align="center"),br(),br(),# sidebarPanel-1-cont
                      ...
                      fluidRow(radioButtons("downSel", c("Select the dataset: "), choices = c("MetaData" = "p_data","Annotations" = "f_data","Expression Counts" = "e_data"),
                                            selected = "p_data")),
                      ...
                      fluidRow(column(4, actionButton('jumpToDegs', label = 'Differential Genes?')),column(4,tags$h5("Or")),
                               column(4, actionButton('jumpToCoex', label = 'Co-expressed Genes?')), align = "center")
