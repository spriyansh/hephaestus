...
Main_Server <- function(input,output,session){
  ...
  output$eset <- renderDT({datatable(values$eset$disp_p_data,rownames = FALSE,selection = "none")})
  
  # Download for fetched datasets
  output$downData <- downloadHandler(
    filename = function() {paste(input$goId, "_",input$downSel, ".csv", sep = "")},
    content = function(file) {fwrite(as.data.frame(values$eset[[input$downSel]]),row.names = TRUE, sep = ",", file)}
  )
  ...
  output$previous <- renderDT({datatable(values$eset$disp_p_data, rownames = FALSE)})
  
  # Selection Event (Gorup1)
  observeEvent(input$experiment, {group1$G1 <- rownames(values$eset$disp_p_data[as.numeric(input$previous_rows_selected),])})
  
  # Reset selections
  observeEvent(input$clear, {output$previous <- renderDT({datatable(values$eset$disp_p_data, rownames = FALSE)})})
  
  # Selection Event (Gorup2)
  observeEvent(input$control, {group2$G2 <- rownames(values$eset$disp_p_data[as.numeric(input$previous_rows_selected),])})
  
  # Print Events
  output$SampleG1 <- renderPrint({group1$G1})
  output$SampleG2 <- renderPrint({group2$G2})
  
  # Cleaning Expression Set
  observeEvent(input$analyze, {clean_df$degs <- analyze(values$eset$e_data, values$eset$f_data, group1$G1, group2$G2, input$pvalue, input$apvalue)})
  
  # Download Output
  output$donwloadDegs <- downloadHandler(
    filename = function() {
    paste(input$goId, "Differential_Genes", ".csv", sep = "")},
    content = function(file) {fwrite(clean_df$degs, sep = ",", file,row.names = FALSE)})
  ...
  # Rendering table for display (DEGs)
  output$diff_dt <- renderDT({datatable(clean_df$degs[,c("Gene.symbol","ID","logFC","adj.P.Val")], rownames = FALSE)})
  
  # Render Volcano Plot
  output$volcano <- renderPlotly({make_volcano(clean_df$degs)})
  
  # render Heatmap
  output$heatmap <- renderPlotly({ make_heatmap(clean_df$degs, values$eset$e_data)})
